...
class SquashedGaussianMLPActor(TorchActorModule):
    ...
    def get_log_prob(self, obs, act):
        ...
        pi_distribution = Normal(mu, std)
        logp_act = pi_distribution.log_prob(act).sum(axis=-1)
        logp_act -= (2 * (np.log(2) - act - F.softplus(-2 * act))).sum(axis=1)
...
class MLPActorCritic(nn.Module):
    def __init__(self, observation_space, action_space, hidden_sizes=(256, 256), activation=nn.ReLU):
        ...
        self.q1 = MLPQFunction(observation_space, action_space, hidden_sizes, activation) #critic
